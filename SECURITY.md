...
- Real solving sends the user-confirmed QUBO matrix to the IsingQ-provided signed upload endpoint.
...
- Release binaries must come from the fixed official GitHub repository, be tied to an immutable version, and be verified against the published manifest before execution. Remote `curl | sh` and `irm | iex` installers remain disabled until the release origin and signing public key are fixed.
